...
EN_model = glmnet(x,y,alpha=0.5)

plot(EN_model,xvar="lambda",label=T,lwd=2)
...
beta_coeff = as.matrix(EN_model$beta) # ridge coefficients

plot(eff_lambda,beta_coeff[1,],type="l",col="black",ylim=c(-0.2,0.7),lwd=2,
     xlab=bquote("df("~lambda~")"),ylab="Elastic Net Coefficients")
